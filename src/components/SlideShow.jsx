...
const SlideShow = ({ collection }) => {
  ...
  const aoutomaticChange = () => {
    setTimeout(() => {
      const btn = document.getElementById('SlideShowNextButton');
      btn.click();
      aoutomaticChange();
    }, 3000);
  };
  return (
    <div className='slideShow-container'>
      ...
      <button className='prev' id="SlideShowNextButton" onClick={() => plusSlides(-1)}>
        &#10094;
      </button>
      <button className='next' id="SlideShowNextButton" onClick={() => plusSlides(1)}>
        &#10095;
      </button>
    </div>
  );
